...
import bo.codexd.invoice.engine.api.mapper.siat.data.synchonization.LegendMapper;
...
import bo.gob.impuestos.siat.api.facturacion.sincronizacion.RespuestaListaParametricasLeyendas;
...
@Slf4j
@AllArgsConstructor
public class DataSyncLegendService {

    private final DataSyncProxy dataSyncProxy = new DataSyncProxy();

    public List<LegendDTO> sync(String token, Environment environment, DataSyncRequest request) {
        log.info("Sync Legends environment: {}, request: {}", environment, request);
        ...
        RespuestaListaParametricasLeyendas listaParametricasLeyendas = proxy.sincronizarListaLeyendasFactura(solicitudSincronizacion);
        if (listaParametricasLeyendas.isTransaccion()) {
            return LegendMapper.INSTANCE.toDto(listaParametricasLeyendas.getListaLeyendas());
        } else {
            throw new SiatException("siat sync legends error", "data.sync.legend", listaParametricasLeyendas.getMensajesList());
        }
    }
}
